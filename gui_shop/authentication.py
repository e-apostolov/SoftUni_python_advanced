...
import json
from gui_shop.canvas import app
from gui_shop.helpers import clean_screen
from gui_shop.products import render_products_screen
...
def register(**user):
    ...
    if len(user["username"]) < 4:
        render_register_screen(error="User name must be at least 4 characters!")
        return
    ...
    for character in user["password"]:
    ...
    if not all(pass_validation_mapper.values()):
        render_register_screen(error="Password does not meet password standards")
        return

    user.update({"products": []})
    with open("db/users_credentials_db.txt", 'r+', newline='\n') as file:
...
def login(username, password):
    with open("db/users_credentials_db.txt", "r") as file:
        data = file.readlines()
        for line in data:
